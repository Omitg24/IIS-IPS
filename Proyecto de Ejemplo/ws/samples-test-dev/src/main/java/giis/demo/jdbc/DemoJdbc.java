...
/**
 * Ejemplos de acceso a una base de datos con conexion JDBC y base de datos
 * Sqlite
 */
public class DemoJdbc {
	...
	/**
	 * Demo basico de acceso a bases de datos, parte 1: conexiones, consultas y
	...
	public void demo1Basic() {
		try {
			...
			// el driver (en este caso sqljdbc.jar ha sido descargado y puesto en el
			// classpath)
			// Esto normalmente no es necesario pues a partir de JDBC 4.0 los drivers se
			// autoregistran
			Class.forName(DRIVER);
			// Definicion de la cadena de conexion, especifica para cada gestor de base de
			// datos
			...
			try {
				...
				// ignora excepcion, que se causara si la tabla no existe en la bd (p.e. al
			...
			while (rs.next()) { // cada vez que se llama rs.next() avanza el cursor a una fila
				int id = rs.getInt(1); // obtencion de un valor con un tipo de dato especificado, indicando el numero
										// de columna
			...
			throw new UnexpectedException(e); // Es mas habitual usar excepciones propias de la aplicacion
												// (DemoException que RuntimeException)
		}
	}
	...
	private void createTable() {
		// el try with resources nos va a asegurar que los objetos cn y stmt siempre se
		// cierran haya o no excepcion
	...
	/**
	 * Demo basico de acceso a bases de datos, parte 2: Uso de he try-with-resources
	 * Statement para manejar excepciones y cerrar de forma segura los recursos (Las
	 * mismas acciones que el anterior pero con mejor control de excepciones) y
	 * consultas con parametros
	 */
	public void demo2TryWithResources() {
		createTable();
		// En un mismo try se pueden poner diferentes sentencias que crean objetos que
		// gestionan recursos que hay que cerrar
		try (Connection cn = DriverManager.getConnection(URL); // NOSONAR
	...
	/**
	 * Demo de acceso a bases de datos, parte 3: Consultas con parametros. Permite
	 * definir consultas sql donde alguno de los valores no son conocidos de moemnto
	...
	public void demo3Parameters() {
		...
		// se crea un PreparedStatement con el sql, luego se le ponen los parametros y
	...
	/**
	 * Demo de acceso a bases de datos, parte 3: Uso de Apache commons-dbutils. Las
	 * librerias Apache Commons https://commons.apache.org/ incluyen muchos
	 * componentes que facilitan y amplian funcionalidades estandar en Java. Una de
	 * ellas es DbUtils que permite simplificar el acceso y recuperacion de datos
	 * procedentes de una base de datos que se muestra en este ejemplo: El acceso a
	 * la base de datos con DbUtils se basa en el uso de objetos QueryRunner que
	 * realizan las consultas y handlers que indican como se manejaran los datos.
	 */
	public void demo4DbUtils() {
		createTable();
		// El siguiente ejemplo define un handler que obtendra los resultados de una
		// consulta
		// como una lista de objetos (POJOs/Beans). Comparar este codigo con lo que
		// seria necesario
		...
		try {
			...
			// declara el handler que permitira obtener la lista de objetos de la clase
			// indicada
			BeanListHandler<Entity> beanListHandler = new BeanListHandler<>(Entity.class);
			// Declara el runner que ejecutara la consulta
			QueryRunner runner = new QueryRunner();
			// ejecuta la consulta, el ultimo argumento es el parametro (lista variable si
			// hay mas de uno)
		...
		// El siguiente ejemplo muestra lo mismo pero usando un MapListHandler que
		// obtiene una lista de maps
		...
		// Existen otros tipos de handlers para manejar otros tipos de valores como
		// escalares, arrays o listas,
